...
def send_no_new_jobs_email(total_jobs: int, excel_path: str) -> None:
    """Send a daily 'no new jobs' summary email via Resend. No-ops if not configured."""
    api_key = os.environ.get("RESEND_API_KEY", "")
    recipient = os.environ.get("NOTIFY_EMAIL", "")
    if not api_key or not recipient:
        logger.debug("[NOTIFY] Resend not configured — skipping no-new-jobs notification")
        return
    ...
    <html><body style="font-family:Arial,sans-serif;color:#333;max-width:860px;margin:auto">
      <h2 style="color:#2E4057">No New Jobs Today</h2>
      <p>The scraper ran its daily check and found no new entry-level SWE roles that haven't already been sent to you.</p>
    ...
    try:
        resend.Emails.send({
            ...
            "to": [recipient],
            "subject": "📭 No new jobs today – Entry-Level SWE Scraper",
            "html": html,
        })
        logger.info("[NOTIFY] No-new-jobs summary sent to %s", recipient)
    except Exception as e:
        logger.warning("[NOTIFY] Failed to send no-new-jobs email: %s", e)
