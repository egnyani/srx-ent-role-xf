...
PROGRAM_TECH_KEYWORDS = [
    "engineer", "developer", "software", "data", "technology", "technical",
    "platform", "cloud", "ml", "ai", "analyst",
]
_RESTRICTED_WORK_PATTERNS = [
    re.compile(r"\bno\s+(?:visa\s+)?sponsorship\b", re.I),
    re.compile(r"\bnot\s+eligible\s+for\s+(?:visa\s+)?sponsorship\b", re.I),
    re.compile(r"\bunable\s+to\s+sponsor\b", re.I),
    re.compile(r"\bwill\s+not\s+sponsor\b", re.I),
    re.compile(r"\bno\s+employment\s+sponsorship\b", re.I),
    re.compile(r"\bus\s+citizens?\s+only\b", re.I),
    re.compile(r"\bonly\s+us\s+citizens?\b", re.I),
    re.compile(r"\bmust\s+be\s+(?:a\s+)?us\s+citizen\b", re.I),
    re.compile(r"\bus\s+citizenship\s+required\b", re.I),
    re.compile(r"\bcitizenship\s+required\b", re.I),
    re.compile(r"\bsecurity\s+clearance\s+required\b", re.I),
    re.compile(r"\bactive\s+security\s+clearance\b", re.I),
    re.compile(r"\bactive\s+(?:top\s+secret|secret|ts/sc[i|l]?|ts)\b", re.I),
    re.compile(r"\btop\s+secret\b", re.I),
    re.compile(r"\bsecret\s+clearance\b", re.I),
    re.compile(r"\bts/sc[i|l]?\b", re.I),
    re.compile(r"\bclearance\s+required\b", re.I),
    re.compile(r"\bclearance\s+eligible\b", re.I),
]
...
def has_swe_keyword(job: dict) -> bool:
    title = (job.get("job_title") or "").lower()
    return any(kw in title for kw in SWE_KEYWORDS)


def has_seniority_keyword(job: dict) -> bool:
    title = (job.get("job_title") or "").lower()
    return any(kw in title for kw in SENIORITY_KEYWORDS)


def requires_too_much_experience(job: dict) -> bool:
    desc = (job.get("description_text") or "").lower()
    return bool(desc) and _requires_too_much_experience(desc)


def has_work_auth_restriction(job: dict) -> bool:
    """Return True when the job explicitly excludes sponsorship or requires citizenship/clearance."""
    title = (job.get("job_title") or "")
    desc = (job.get("description_text") or "")
    location = (job.get("location") or "")
    combined = f"{title}\n{desc}\n{location}"
    return any(pattern.search(combined) for pattern in _RESTRICTED_WORK_PATTERNS)


def is_new_grad_program(job: dict) -> tuple[bool, str]:
    """Return True for explicit university / rotational technical programs."""
    title = (job.get("job_title") or "").lower()
    desc = (job.get("description_text") or "").lower()
    combined = f"{title} {desc}"
    if has_seniority_keyword(job):
        return (False, "")
    if requires_too_much_experience(job):
        return (False, "")
    if has_work_auth_restriction(job):
        return (False, "")
    for signal in NEW_GRAD_PROGRAM_SIGNALS:
        if signal in combined and any(token in combined for token in PROGRAM_TECH_KEYWORDS):
            return (True, signal)
    return (False, "")
...
def is_entry_level_swe(job: dict) -> tuple[bool, str]:
    ...
    # Step 1: reject non-English titles only when the location also looks non-US.
    if _is_non_english_title(job.get("job_title") or "") and not is_us_location(job):
        return (False, "")
    ...
    # Step 5: reject jobs that explicitly disallow sponsorship or require clearance/citizenship
    if has_work_auth_restriction(job):
        return (False, "")

    # Step 6: check for explicit entry-level signals (title or description)
    for signal in ENTRY_LEVEL_SIGNALS:
        if signal in combined:
            return (True, signal)

    # Step 7: no explicit signal but no seniority either — include the role
...
def is_interesting_match(job: dict) -> tuple[bool, str]:
    """
    Broad bucket for technical US roles that are worth reviewing manually.

    Intended for jobs that are:
      - US-based
      - technical
      - not clearly senior
      - not strict entry-level matches
    """
    ...
    technical = (
        has_swe_keyword(job)
        or any(sig in title for sig in _SKILL_FALLBACK_TITLES)
        or any(sig in combined for sig in NEW_GRAD_PROGRAM_SIGNALS)
        or _skill_match_score(desc) >= 2
    )
    if not technical:
        return (False, "")

    strict_match = (
        is_entry_level_swe(job)[0]
        or is_skill_match(job)[0]
        or is_new_grad_program(job)[0]
    )
    if strict_match:
        return (False, "")
    return (True, "interesting-technical-match")


def rejection_reason(job: dict) -> str:
    """Return the primary reason the job failed strict/interesting buckets."""
    if not is_us_location(job):
        return "non-US"
    if has_work_auth_restriction(job):
        return "work authorization / clearance restricted"
    if has_seniority_keyword(job):
        return "senior title"
    if requires_too_much_experience(job):
        return "experience too high"
    if not has_swe_keyword(job):
        desc = (job.get("description_text") or "").lower()
        if _skill_match_score(desc) < _SKILL_MATCH_THRESHOLD:
            return "weak skill match"
        return "no SWE keyword"
    return "other"


def classify_job(job: dict) -> tuple[str, str]:
    """Return (bucket, reason) where bucket is strict, interesting, or rejected."""
    if not is_us_location(job):
        return ("rejected", "non-US")
    new_grad_ok, new_grad_reason = is_new_grad_program(job)
    if new_grad_ok:
        return ("strict", f"new-grad-program({new_grad_reason})")
    entry_ok, entry_reason = is_entry_level_swe(job)
    if entry_ok:
        return ("strict", entry_reason)
    skill_ok, skill_reason = is_skill_match(job)
    if skill_ok:
        return ("strict", skill_reason)
    interesting_ok, interesting_reason = is_interesting_match(job)
    if interesting_ok:
        return ("interesting", interesting_reason)
    return ("rejected", rejection_reason(job))
